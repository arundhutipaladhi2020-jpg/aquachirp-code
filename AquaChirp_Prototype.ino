...
const float TURB_AMP_MIN = 0.4;   // clear water: less power needed
const float TURB_AMP_MAX = 1.0;   // muddy water: full power to penetrate


const float BATT_LOW_THRESHOLD   = 30.0;   // percent
const float BATT_FLOOR_FRACTION  = 0.3;
...
// --- (4) Hardware timer: ISR only sets a flag, never touches I2C ---
...
int sampleIndex = 0;
int pulseN = N_SAMPLES;
float pulseF0, pulseF1, pulseBaseFreq, pulseTs, pulseAmpScale;
WaveformType pulseWaveform;
WindowType pulseWindow;

void startPulse(float f0, float f1, float baseFreq, float Ts, float ampScale, WaveformType wf, WindowType win) {
  pulseF0 = f0; pulseF1 = f1; pulseBaseFreq = baseFreq; pulseTs = Ts;
  pulseAmpScale = ampScale; pulseWaveform = wf; pulseWindow = win;
  sampleIndex = 0;
  transmitting = true;
...
void loop() {
  // Buttons are checked every pass, including mid-transmission now.
  ...
  if (transmitting) {
    if (!sampleTick) {
      __WFI(); // sleep until the next interrupt instead of busy-spinning
      return;
    }
    sampleTick = false;
  ...
  // --- Not transmitting: read inputs and prepare the next pulse ---
  ...
  float turbFraction = (turbidity - TURB_MIN) / (TURB_MAX - TURB_MIN);

  // (3) Turbidity sets the target amplitude; battery then scales it down.
  float ampFromTurbidity = TURB_AMP_MIN + (TURB_AMP_MAX - TURB_AMP_MIN) * turbFraction;

  bool lowBattery = battery < BATT_LOW_THRESHOLD;
  float battScaleFactor = 1.0f;
  unsigned long idlePauseMs = 0;
  if (lowBattery) {
    float batteryFraction = battery / BATT_LOW_THRESHOLD;
    battScaleFactor = BATT_FLOOR_FRACTION + (1.0f - BATT_FLOOR_FRACTION) * batteryFraction;
    idlePauseMs = (unsigned long)((1.0f - batteryFraction) * IDLE_PAUSE_MAX_MS);
  }
  float ampScale = ampFromTurbidity * battScaleFactor;

  float delayUs = DELAY_US_CLEAR + (DELAY_US_MUDDY - DELAY_US_CLEAR) * turbFraction;
...
display.display();
  ...
  if (idlePauseMs > 0) {
    delay(idlePauseMs); // deliberate, genuine CPU rest at low battery
  }

  startPulse(f0, f1, baseFreqHz, Ts, ampScale, currentWaveform, currentWindow);
}
